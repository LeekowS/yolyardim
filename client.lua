RegisterCommand("yolyardım", function(source, args)   -- 'yolyardım' yazan komutu değiştirerek istediğin komutu yapabilirsin.
...
function GoToTarget(x, y, z, vehicle, driver, vehhash, target)
    ...
    ShowAdvancedNotification(companyIcon, companyName, "Yetkili Birime Ulaşıldı", "Yolda kaldığın noktaya yol yardımı gönderildi. ~y~" .. companyName)  ---- Yardım istediği zaman giden mesaj ---
    enroute = true
    while enroute do
        Citizen.Wait(500)
...
function RepairVehicle(target, vehicle, driver)
    ...
    Citizen.Wait(500)
	ShowAdvancedNotification(mechPedPick.icon, mechPedPick.name, "Yol Yardım" , mechPedPick.lines[math.random(#mechPedPick.lines)])   ----- Hizmet adı -----
...
function GetTargetVehicle(player, dir)
    ...
    if DoesEntityExist(dmgVeh) then
        ...
        ShowNotification("Yol yardımı çağırabilmek için aracın yakınında olmalısın.")       --- Aracın yanında değilse giden mesaj ----
    end
end
